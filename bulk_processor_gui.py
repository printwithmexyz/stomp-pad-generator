...
# todo:
# 1. fix threading for svg generation. it appears to be sequential still
# 2. package as executeable program/installable program for win/mac/linux
# 3. duplicate the info in the console that is generated by the python console output. the information is interesting to see.
# 4. add an option to render the openscad files after generation to speed up the processes
...
class BulkProcessorGUI:
    ...
    def process_all_svgs(self):
        ...
        # Force single thread when preview is enabled to avoid multiple dialogs
        ...
        self.log(f"Using {num_threads} thread(s) for parallel processing")
        self.update_status(f"Processing {len(svg_files)} files...", 'blue')

        # Lock for thread-safe counter updates
        counter_lock = threading.Lock()
        files_processed = [0]  # Using list for mutable reference in closure
        files_completed = [0]

        def process_file_wrapper(svg_file, file_index):
            """Wrapper function for thread pool execution"""
            if self.stop_requested:
                return None, svg_file.name, "skipped"

            try:
                self.log(f"\n{'='*60}")
                self.log(f"[Thread {threading.current_thread().name}] Processing: {svg_file.name}")
                self.log(f"{'='*60}")
                ...
                with counter_lock:
                    files_processed[0] += 1
                    files_completed[0] += 1
                    progress = (files_completed[0] / len(svg_files)) * 100
        ...
        with ThreadPoolExecutor(max_workers=num_threads, thread_name_prefix="SVGWorker") as executor:
            # Submit all tasks
            futures = {
                executor.submit(process_file_wrapper, svg_file, i): svg_file
            ...
            for future in as_completed(futures):
                ...
                if error and error != "skipped":
                    self.log(f"ERROR processing {filename}: {error}")

        # Handle stop request
        if self.stop_requested:
            self.log(f"\n{'='*60}")
            self.log(f"Processing stopped by user")
            self.log(f"Processed {files_processed[0]}/{len(svg_files)} files")
            self.log(f"{'='*60}")
            self.update_status(f"Stopped ({files_processed[0]}/{len(svg_files)} completed)", 'orange')
            return
        ...
        self.log(f"Processed {files_processed[0]}/{len(svg_files)} files successfully")
        self.log(f"{'='*60}")
    ...
    def process_single_svg(self, svg_file, output_path, cache_path):
        ...
        if self.preview_debug.get():
            ...
            save_debug_visualization(polygon, skeleton_points, valid_positions, str(debug_viz_path))
