...
// SVG transformation to match Python's coordinate system
svg_offset_x = -19.344844730180952;  // Translate X
svg_offset_y = -3.6362945899477435;  // Translate Y
svg_scale_factor = 0.17525123284359476;

// IMPORTANT: Pyramid positions calculated after SVG was moved to origin
// The svg_offset values above reposition the SVG to match
show_bounds = false;  // Set true to visualize coordinate system
...
module scaled_svg() {
    // Simple resize - let OpenSCAD handle the SVG positioning
...
module stomp_pad_complete() {
    union() {
        ...
        // Grip pyramids - constrained to base shape
        intersection() {
            pyramids_on_surface();

            // Constrain pyramids to only appear on the base
            translate([0, 0, total_height - 0.1]) {
                linear_extrude(height = pyramid_height + 1) {
                    base_shape_2d();
                }
            }
        }
...
// Debug: Show origin marker if enabled
if (show_bounds) {
    color("red") translate([0, 0, 0]) cube([5, 5, 5]);  // Origin marker
    color("blue") translate([target_width, 0, 0]) cube([5, 5, 5]);  // Width marker
    color("green") translate([0, target_width * 0.75, 0]) cube([5, 5, 5]);  // Height marker
}
